...
export const VERSION_MATCH_GROUPS = Object.freeze(
	Array.from(new Set(Object.keys(VersionMatchGroup))),
);

/**
 * Checks that an `unknown` value is a {@link VersionMatchGroup}.
 *
 * Requirements:
 *   - `value` must be a non-empty string and must be included as a value of `VERSION_MATCH_GROUPS`.
 *
 * @param value - An `unknown` value.
 *
 * @returns The determination that `value` is or is not a {@link VersionMatchGroup}.
 */
export const isVersionMatchGroup = (
	value: unknown,
): value is VersionMatchGroup =>
	/**
	 * value
	 */
	isString(value) &&
