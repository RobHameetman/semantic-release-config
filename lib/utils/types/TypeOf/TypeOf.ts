 ...
 * @example
 * ```TypeScript
 * const type: TypeOf<'1.3.5-alpha.7'> = 'alpha';
 *
 * // Error: Type '"alpha.7"' is not assignable to type 'TypeOf<"1.3.5-alpha.7">'. Did you mean '"alpha"'?
 * const type: TypeOf<'1.3.5-alpha.7'> = 'alpha.7';
 * ```
 */
...
/**
 * Checks that an `unknown` value is a {@link TypeOf<T>}.
 *
 * Requirements:
 ...
 *
 * @returns The determination that `value` is or is not a {@link TypeOf<T>}.
 */
export const isTypeOf = <T extends string>(
	...
	(version
		? value === (version.match(SEMVER_REGEX) ?? []).at(VersionMatchGroupNames.type)
		: true);
