 ...
 * @example
 * ```TypeScript
 * const patch: PatchOf<'1.3.5-alpha.7'> = '5';
 *
 * // Error: Type '"3"' is not assignable to type '"5"'.
 * const patch: PatchOf<'1.3.5-alpha.7'> = '3';
 * ```
 */
export type PatchOf<T extends string> =
...
/**
 * Checks that an `unknown` value is a {@link PatchOf<T>}.
 *
 * Requirements:
 ...
 *
 * @returns The determination that `value` is or is not a {@link PatchOf<T>}.
 */
export const isPatchOf = <T extends string>(
	value: unknown,
	version?: string,
): value is PatchOf<T> =>
	/**
