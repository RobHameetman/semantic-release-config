 ...
 * @example
 * ```TypeScript
 * const prerelease: PrereleaseOf<'1.3.5-alpha.7'> = '7';
 *
 * // Error: Type '"6"' is not assignable to type 'PrereleaseOf<"1.3.5-alpha.7">'.
 * const prerelease: PrereleaseOf<'1.3.5-alpha.7'> = '6';
 * ```
 */
export type PrereleaseOf<T extends string> =
...
/**
 * Checks that an `unknown` value is a {@link PrereleaseOf<T>}.
 *
 * Requirements:
 ...
 *
 * @returns The determination that `value` is or is not a {@link PrereleaseOf<T>}.
 */
export const isPrereleaseOf = <T extends string>(
	value: unknown,
	version?: string,
): value is PrereleaseOf<T> =>
	/**
	 * value
	 */
