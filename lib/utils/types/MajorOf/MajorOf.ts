...
type PrereleaseVersion = `${ReleaseVersion}-${string}.${number}` | `0.${number}.${number}`;
type PrPrereleaseVersion<T extends string = 'pr'> = `${ReleaseVersion}-${T}.${number}.${string}.${number}.${number}`;
 ...
 * @example
 * ```TypeScript
 * const major: MajorOf<'1.3.5-alpha.7'> = '1';
 *
 * // Error: Type '"5"' is not assignable to type '"1"'.
 * const major: MajorOf<'1.3.5-alpha.7'> = '5';
 * ```
 */
export type MajorOf<T extends string> =
...
/**
 * Checks that an `unknown` value is a {@link MajorOf<T>}.
 *
 * Requirements:
 ...
 *
 * @returns The determination that `value` is or is not a {@link MajorOf<T>}.
 */
export const isMajorOf = <T extends string>(
	value: unknown,
	version?: string,
): value is MajorOf<T> =>
	/**
	...
	!isNaN(Number(value)) &&
	(version
		? value === (version.match(SEMVER_REGEX) ?? []).at(VersionMatchGroupNames.major)
		: true);
