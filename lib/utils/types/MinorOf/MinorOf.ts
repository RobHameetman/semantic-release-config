 ...
 * @example
 * ```TypeScript
 * const minor: MinorOf<'1.3.5-alpha.7'> = '3';
 *
 * // Error: Type '"1"' is not assignable to type '"3"'.
 * const minor: MinorOf<'1.3.5-alpha.7'> = '1';
 * ```
 */
export type MinorOf<T extends string> =
...
/**
 * Checks that an `unknown` value is a {@link MinorOf<T>}.
 *
 * Requirements:
 ...
 *
 * @returns The determination that `value` is or is not a {@link MinorOf<T>}.
 */
export const isMinorOf = <T extends string>(
	value: unknown,
	version?: string,
): value is MinorOf<T> =>
	/**
	...
	!isNaN(Number(value)) &&
	(version
		? value === (version.match(SEMVER_REGEX) ?? []).at(VersionMatchGroupNames.minor)
		: true);
