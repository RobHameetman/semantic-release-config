...
const TO_BE_A_PATCH_RELEASE = 'toBeAPatchRelease';

/**
 * Checks that a provided semantic version string is a patch release.
 */
expect.extend({
	[TO_BE_A_PATCH_RELEASE](received) {
		const {
			RECEIVED_COLOR,
			matcherErrorMessage,
			matcherHint,
		...
		if (!isCommitMatchGroups(commit)) {
			throw new Error(
				matcherErrorMessage(
					matcherHint(TO_BE_A_PATCH_RELEASE, undefined, '', options),
		...
		const pass = type === 'patch';

		const message = (): string =>
			`${matcherHint(TO_BE_A_PATCH_RELEASE, 'received', '', options)}\n\n` +
